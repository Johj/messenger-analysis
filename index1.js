...
const messagesByHour = require('./util/messagesByHour.js');
...
const name = 'messagesByHour';
const processed_data = processData(data, name, messagesByHour);
...
for (let index = 0; index < 10; ++index) {
  ...
  `]`;
  const xAxis = `"hour"`;
  const xAxisLabel = `"Hour of the Day"`;
  const yAxisLabel = `"Frequency"`;
  const title = `"Messages by Hour of the Day"`;
  const rotate = 0;
  const type = `"bar"`;
