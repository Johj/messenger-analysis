...
const sentimentByMonth = require('./util/sentimentByMonth.js');
...
const name = 'sentimentByMonth';
const processed_data = processData(data, name, sentimentByMonth);
...
for (let index = 0; index < 10; ++index) {
  ...
  const yAxisLabel = `"Average Sentiment Score"`;
  const title = `"Average Message Sentiment Score by Month"`;
  const rotate = 90;
  const type = `"line"`;
